...
if Density == 1:
    ...
    if (MakePlot == 1) and (PlotTZ==1 or PlotXZ==1):
        ...
        if Modulated == 0: PlotTitle=r'$\rho^{\prime}$ (kg m$^{-3}$)'
...
if Density_2 == 1:
    ...
    if (MakePlot == 1) and (PlotTZ==1 or PlotXZ==1):
        ...
        PlotTitle=r'$\rho^{\prime}_{\diamond}$ (kg m$^{-3}$)'
    ...
    for tt in range(0,Nt):
        ...
        if w2f_analysis == 1 and noSigmas == 0:
            ...
            if tt == 0 and Modulated == 0:
                if nrMolecularDiff: dir_ivec = './Results/' + RunName + '/NaturalBasis/'
                else: dir_ivec = './Results_bigNu/' + RunName + '/NaturalBasis/'
                #Create directory if it doesn't exist:
...
if MakePlot >= 1:
    ...
    if PlotStairStartEnd == 1:
        ...
        dat = np.loadtxt('/lustre/home/pb412/dedalus/stairStartEnd.txt')
        #print(dat[0,:])
        idxN = np.where(dat[0,:]==np.sqrt(N2))
    ...
    if MakeMovie != 1:
        ...
        if MakePlot >= 1:
            ...
            if PlotTZ >= 1:
                ...
                ax1.set_xlim(xlim)
                if tAxisMins: ax1.set_xlabel(r'$t$ (min)')
        ...
        if 'data2' in locals():
            ...
            if PlotTZ >= 1:
                ax2.set_title(PlotTitle2)
                #ax2.set_xlabel(r'$t$ (s)')
                if tAxisMins: ax2.set_xlabel(r'$t$ (min)')
                else: ax2.set_xlabel(r'$t$ (s)')
                ax2.set_ylabel(r'$z$ (m)')
        ...
        if savePlots == 1:
            ...
            if PlotTZ == 1: 
                if nrMolecularDiff == 1:
                    ...
                    plt.savefig(FigNmBase + RunName + '_tz_' + str(StartMin) + '_' + str(nfiles)\
                                 + '_bigNu' + '.eps')
