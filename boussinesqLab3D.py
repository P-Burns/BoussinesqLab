...
m = PeriodicRectangleMesh(nx, ny, Lx, Ly, quadrilateral=True)
...
if ICsNon0 == 1:
    ...
    if ICsRandom == 1:
        ...
        def ExternalDataPoint(data, x, y, Nx, Ny, Lx, Ly):
            dx = Lx/Nx
            dy = Ly/Ny
        ...
        def mydata(X):
            ...
            for (x, y, z) in X:
                list_of_output_values.append(ExternalDataPoint(RandomSample, x, z, nx, nlayers, Lx, H))
...
if ZeroDiffusion != 1:
    ...
    delta = Lx/nx		#Grid resolution (same in both directions).
